...
import { PullOracleClientConfig } from "./pullOracleClientConfig";
...
export class pullOracleClient {
     ...
     */
    public async getCreateOracleIx(feedId: string, uiPrice: number, expo: number, conf?: number): Promise<[TransactionInstruction[], Keypair]> {
     ...
     */
	public async createOracle(feedId: string, uiPrice: number, expo: number, conf?: number): Promise<[string, PublicKey]> {
		const [createOracleIx, priceFeed] = await this.getCreateOracleIx(feedId, uiPrice, expo, conf);
